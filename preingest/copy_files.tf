locals {
  copy_files_name           = "${local.environment}-dr2-copy-files-from-${var.source_name}"
  copy_files_queue_arn      = "arn:aws:sqs:eu-west-2:${data.aws_caller_identity.current.account_id}:${local.copy_files_name}"
...
module "dr2_copy_files_lambda" {
  source          = "git::https://github.com/nationalarchives/da-terraform-modules//lambda"
  function_name   = local.copy_files_name
  handler         = "lambda_function.lambda_handler"
  timeout_seconds = local.python_timeout_seconds
  lambda_sqs_queue_mappings = [
    { sqs_queue_arn = local.copy_files_queue_arn, ignore_enabled_status = true }
  ]
  policies = {
    "${local.copy_files_name}-policy" = var.bucket_kms_arn == null ? templatefile("${path.module}/templates/copy_files_no_kms_policy.json.tpl", {
      copy_files_queue_arn  = local.copy_files_queue_arn
      ...
      lambda_name           = local.copy_files_name
...
module "dr2_copy_files_sqs" {
  source     = "git::https://github.com/nationalarchives/da-terraform-modules//sqs"
  queue_name = local.copy_files_name
...
resource "aws_sns_topic_subscription" "dr2_copy_files_subscription" {
  count                = var.sns_topic_arn != null ? 1 : 0
  endpoint             = module.dr2_copy_files_sqs.sqs_arn
  protocol             = "sqs"
  topic_arn            = var.sns_topic_arn
