locals {
  ingest_start_workflow_lambda_name_old = "${local.environment}-ingest-start-workflow"
...
module "ingest_start_workflow_lambda" {
  ...
  function_name   = local.ingest_start_workflow_lambda_name_old
  handler         = "uk.gov.nationalarchives.Lambda::handleRequest"
  timeout_seconds = 60
  policies = {
    "${local.ingest_start_workflow_lambda_name_old}-policy" = templatefile("./templates/iam_policy/ingest_start_workflow_policy.json.tpl", {
      account_id                 = var.account_number
      lambda_name                = local.ingest_start_workflow_lambda_name_old
  ...
  tags = {
    Name      = local.ingest_start_workflow_lambda_name_old
