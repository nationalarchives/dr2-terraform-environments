locals {
  ...
  custodial_copy_database_builder_queue_name = "${local.environment}-dr2-custodial-copy-builder"
  custodial_copy_queue_name                  = "${local.environment}-dr2-custodial-copy"
}
...
resource "aws_iam_group_policy" "custodial_copy_group_policy" {
  ...
  policy = templatefile("${path.module}/templates/iam_policy/custodial_copy_policy.json.tpl", {
    ...
    entity_event_queue         = module.dr2_custodial_copy_notifications_queue.sqs_arn
...
module "dr2_database_builder_queue" {
  ...
  queue_name = local.custodial_copy_database_builder_queue_name
