...
module "ingest_check_preservica_for_existing_io_lambda" {
  ...
  function_name   = local.ingest_check_preservica_for_existing_io_lambda_name_old
  handler         = "uk.gov.nationalarchives.ingestfindexistingasset.Lambda::handleRequest"
  timeout_seconds = 60
  policies = {
    "${local.ingest_check_preservica_for_existing_io_lambda_name_old}-policy" = templatefile(
      "./templates/iam_policy/ingest_check_preservica_for_existing_io_policy.json.tpl", {
        account_id                 = var.account_number
        lambda_name                = local.ingest_check_preservica_for_existing_io_lambda_name_old
  ...
  tags = {
    Name      = local.ingest_check_preservica_for_existing_io_lambda_name_old
