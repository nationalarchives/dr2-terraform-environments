locals {
  ingest_asset_reconciler_lambda_name_old = "${local.environment}-ingest-asset-reconciler"
...
module "ingest_asset_reconciler_lambda" {
  ...
  function_name   = local.ingest_asset_reconciler_lambda_name_old
  handler         = "uk.gov.nationalarchives.Lambda::handleRequest"
  timeout_seconds = 60
  policies = {
    "${local.ingest_asset_reconciler_lambda_name_old}-policy" = templatefile("./templates/iam_policy/ingest_asset_reconciler_policy.json.tpl", {
      account_id                 = var.account_number
      lambda_name                = local.ingest_asset_reconciler_lambda_name_old
  ...
  tags = {
    Name = local.ingest_asset_reconciler_lambda_name_old
  }
}
