locals {
  ingest_workflow_monitor_lambda_name_old = "${local.environment}-ingest-workflow-monitor"
...
module "ingest_workflow_monitor_lambda" {
  ...
  function_name   = local.ingest_workflow_monitor_lambda_name_old
  handler         = "uk.gov.nationalarchives.Lambda::handleRequest"
  timeout_seconds = 60
  policies = {
    "${local.ingest_workflow_monitor_lambda_name_old}-policy" = templatefile("./templates/iam_policy/ingest_workflow_monitor_policy.json.tpl", {
      account_id                 = var.account_number
      lambda_name                = local.ingest_workflow_monitor_lambda_name_old
  ...
  tags = {
    Name = local.ingest_workflow_monitor_lambda_name_old
  }
}
