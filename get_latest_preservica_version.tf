locals {
  get_latest_preservica_version_old              = "${local.environment}-get-latest-preservica-version-lambda"
  get_latest_preservica_version                  = "${local.environment}-dr2-get-latest-preservica-version-lambda"
  latest_preservica_version_event_topic_name_old = "${local.environment}-latest-preservica-version-topic"
  ...
  latest_preservica_version_event_topic_arn_old  = "arn:aws:sns:eu-west-2:${data.aws_caller_identity.current.account_id}:${local.latest_preservica_version_event_topic_name_old}"
...
module "get_latest_preservica_version_cloudwatch_event" {
  ...
  schedule                = "cron(0 8 * * ? *)"
  lambda_event_target_arn = "arn:aws:lambda:eu-west-2:${data.aws_caller_identity.current.account_id}:function:${local.get_latest_preservica_version_old}"
}
...
module "get_latest_preservica_version_lambda" {
  ...
  function_name = local.get_latest_preservica_version_old
  ...
  policies = {
    get_latest_preservica_version_event_policy = templatefile("${path.module}/templates/iam_policy/get_latest_preservica_version_lambda_policy.json.tpl", {
      ...
      lambda_name                = local.get_latest_preservica_version_old
      ...
      sns_arn                    = local.latest_preservica_version_event_topic_arn_old
    })
  }
  timeout_seconds = 180
  ...
  plaintext_env_vars = {
    ...
    PRESERVICA_VERSION_EVENT_TOPIC_ARN    = local.latest_preservica_version_event_topic_arn_old
...
module "latest_preservica_version_topic" {
  ...
  sns_policy = templatefile("${path.module}/templates/sns/latest_preservica_version_topic_policy.json.tpl", {
    ...
    sns_topic       = local.latest_preservica_version_event_topic_arn_old
  })
  tags       = {}
  topic_name = local.latest_preservica_version_event_topic_name_old
}
