locals {
  ingest_upsert_archive_folders_lambda_name_old = "${local.environment}-ingest-upsert-archive-folders"
...
module "ingest_upsert_archive_folders_lambda" {
  ...
  function_name   = local.ingest_upsert_archive_folders_lambda_name_old
  handler         = "uk.gov.nationalarchives.Lambda::handleRequest"
  timeout_seconds = 60
  policies = {
    "${local.ingest_upsert_archive_folders_lambda_name_old}-policy" = templatefile("./templates/iam_policy/ingest_upsert_archive_folders_policy.json.tpl", {
      account_id                 = var.account_number
      lambda_name                = local.ingest_upsert_archive_folders_lambda_name_old
  ...
  tags = {
    Name      = local.ingest_upsert_archive_folders_lambda_name_old
