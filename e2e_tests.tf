module "run_e2e_tests_role" {
  ...
  name               = "${local.environment}-run-e2e-tests-role"
  policy_attachments = {
...
module "e2e_tests_ecs_task_role" {
  ...
  name               = "${local.environment}-e2e-tests-task-role"
  policy_attachments = {
...
module "e2e_tests_task_policy" {
  ...
  name   = "${local.environment}-e2e-tests-task-policy"
...
module "e2e_tests_ecs_execution_role" {
  source             = "git::https://github.com/nationalarchives/da-terraform-modules//iam_role"
  assume_role_policy = templatefile("${path.module}/templates/iam_role/service_assume_role.json.tpl", { service = "ecs-tasks" })
  name               = "${local.environment}-e2e-tests-execution-role"
  policy_attachments = {
...
module "e2e_tests_execution_policy" {
  source        = "git::https://github.com/nationalarchives/da-terraform-modules//iam_policy"
  name          = "${local.environment}-e2e-tests-execution-policy"
